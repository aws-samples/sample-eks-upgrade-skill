...
Produce the report as a Markdown artifact using the filename pattern:
`EKS-Upgrade-Assessment-<cluster>-<current>-to-<target>-<YYYY-MM-DD>-<HHMM>.md`

Deliver the report as a generated artifact in the Agent Space (for example, a
weekly/operational summary artifact) or inline in the response. Do NOT execute
scripts to render the report — AWS DevOps Agent Skills do not support script
execution.

## Step 7: HTML Rendering (optional)

If the user wants an HTML version, generate the HTML directly from the Markdown
content in your response. The report structure and inline styling should be
self-contained. Do NOT reference or attempt to run any external conversion
